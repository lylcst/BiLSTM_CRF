...
class CRFLayer(nn.Module):
  def __init__(self, tag_size, tag2idx, start_tag='<START_TAG>', end_tag='<END_TAG>'):
  ...
  def score_sentence(self, feats, tags, mask):
    ...
    transition_to_end = torch.stack([self.transition[self.tag2idx[self.end_tag], tag[mask[:, b].sum().long()]]
